...
Vienas iš pagrindinių sprendžiamų uždavinių yra klasifikacija - procesas, kurio metu yra ieškoma panašių ypatybių tarp skirtingų objektų, pavyzdžiui paveiksliukų, ir jie yra skirstomi į atitinkamas klases \cite{classificationDef}. Tokį uždavinį gali atlikti dirbtinių neuroninių 
...
kadangi dirbtiniai neuroniniai tinklai yra matematinis algoritmas su aritmetiniais kintamaisiais. Algoritmas yra suvokiamas 
...
funkcijai, jis yra perduodamas tolimesniems neuronams \cite{shiffman2012nature}. Tokio tipo veikimo projektavimas yra vadinamas ,,feedforward'' tinklu.
...
būti lanksčiai. Nors tiesinė funkcija labai dažnai naudojama išvesties sluoksnyje.
...
\item Tiesinė: 
\begin{itemize}
\item Žingsninė (angl. step function) - išėjimas yra 0 arba 1.
\end{itemize}
\item Netiesinė: 
\begin{itemize}
\item Sigmoidinė (angl. sigmoid function) - išėjimas intervale [0; 1].
\item Hiperbolinio tangento (angl. hyperbolic tangent) - išėjimas intervale [-1; 1].
\item Minkštojo maksimumo (angl. softmax function) - sunormuoja išėjimo vektorių į 1.
...
tinklas''). Tačiau pagrindinis skirtumas tarp šių tinklų yra, kad konvoliucinio įvesties sluoksnis priima tik tai paveiksliukus, 
...
Paveiksliukų analizėje, statinė ir pagrindinė funkcija yra įvesties paveiksliukas, kuris yra analizuojamas, o antroji, judanti funkcija, žinoma 
...
\item Žingsnis - pikselių kiekis, kuris parodo per kiek reikia slinkti filtro matrica per įvesties informacijos matricą.
\item Nulių pamušalas - įvesties informacijos matricos kraštus užpildyti nuliais.
\end{itemize}
...
gabalėlio gylio ir keičia jo dydį erdviškai, naudodamas MAX operaciją \cite{7927440}. Dažniausiai šis sluoksnis yra naudojamas su 2x2 dydžio filtru - kas antras po konvoliucijos 
...
O kitas dažniau naudojamas turi filtro dydį lygų 2 ir žingsnį taip pat 2. Sujungimo sluoksniai su labai dideliais parametrais yra destruktyvūs.

\subsubsection{Pilno sujungimo sluoksnis}
Konvoliucinio ir sujungimo sluoksnių išeitys yra aukšto lygio ypatybės, kurios yra gautos iš įvesties paveiksliuko. Pilno sujungimo sluoksnis yra sujungtas su visais 
...
klasėms priklauso įvesties paveiksliukas pagal mokymo informacijos rinkinį, kai neuroninio tinklo problema yra klasifikacija \cite{layers-fullyconnected}. Jei šiam sluoksniui yra naudojama 
...
vektorių, kuriame yra klasių tikimybių įvertinimai intervale nuo 0 iki 1, kur tikimybė arčiausiai vieneto reiškia, kad yra užtikrintas dėl tos klasės.
...
\hline
ResNet & 2015 & 25 000 000 & Turi labai daug sluoksnių, sudėtų vienas po kito, kurie turi liekamąjį bloką, kuris įvesties informaciją perduoda tolimesniam sluoksniui ją pridėdamas ir taip sumažina konvoliucijos ir aktyvavimo funkcijų kiekį.  & pirma \\
\hline
\end{longtable}
